	...
	if (check_api_token(api_key) == True):
		...
		if user_id is not None:
			...
			if ((genres == ' ') | (genres == '%20')):
				...
				result = db.movie.find({'imdbID': {'$nin': imdb_list}}).skip(randrange(0, remaining_count))[0] # Filter out the list of previously voted for movies, sorted by imdb vote data (rating, quantity votes), takes top 1.
				...
				if (remaining_count > 0):
					...
					if (genre_list_check == True):
						result = db.movie.find({'imdbID': {'$nin': imdb_list}, "genre": {'$all': genres}}).skip(randrange(0, remaining_count))[0]
					else:
						result = db.movie.find({'imdbID': {'$nin': imdb_list}, "genre": genres}).skip(randrange(0, remaining_count))[0]
...
def get_random_movie_list(gg_id: hug.types.text, api_key: hug.types.text, hug_timer=5):
	...
	if (check_api_token(api_key) == True):
		...
		if user_id is not None:
			...
			results = list(db.movie.find().skip(randrange(0, result_count)).limit(10)) # .sort([('imdbVotes', -1), ('imdbRating', -1)])
